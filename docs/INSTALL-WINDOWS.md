...
RackSight 1.1.5 is distributed as a Windows x64 installer. The release file is Authenticode-signed by **AUTHORITYGATE INC** and timestamped by GlobalSign.
...
```powershell
.\RackSight-Setup-1.1.5-x64.exe /S /RACKSIGHTEMAIL=user@example.com
```
...
```powershell
Get-AuthenticodeSignature .\RackSight-Setup-1.1.5-x64.exe |
...
```powershell
Get-FileHash .\RackSight-Setup-1.1.5-x64.exe -Algorithm SHA256
```
